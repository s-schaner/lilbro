...
import { EventMarker, InsightPayload, Player } from '@lib/types';

export type ImageAnalysisEntry = {
  id: string;
  timestamp: number;
  focus: string;
  summary: string;
...
type Props = {
  ...
  imageAnalyses: ImageAnalysisEntry[];
};

const tabs = ['Players', 'Events', 'Formation', 'Insights (AI)', 'Image Analysis'] as const;

type Tab = (typeof tabs)[number];

const RightPanel: FC<Props> = ({ players, events, onSeek, insights, imageAnalyses }) => {
  ...
  return (
    <aside className="flex h-full w-80 flex-col gap-4 rounded-2xl border border-slate-800/60 bg-slate-900/60 p-4">
      ...
      <div className="flex-1 overflow-hidden">
        ...
        {activeTab === 'Formation' ? (
          <ModuleBoundary moduleId="overlays">
        ...
        {activeTab === 'Insights (AI)' ? (
          <ModuleBoundary moduleId="llm-insights">
            <div className="flex h-full flex-col gap-3 overflow-y-auto rounded-xl border border-slate-800 bg-slate-950/60 p-4 text-sm text-slate-200">
              {insights ? (
                <>
                  <div>
                    <p className="text-sm font-semibold text-white">Match Summary</p>
                    <p className="mt-1 text-slate-300">{insights.summary}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Spotlights</p>
                    <ul className="mt-1 list-disc space-y-1 pl-4 text-xs text-slate-300">
                  ...
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Coach Tips</p>
                    <ul className="mt-1 list-disc space-y-1 pl-4 text-xs text-slate-300">
                      {insights.coach_tips.map((item) => (
                        <li key={item}>{item}</li>
                      ))}
                    </ul>
                  </div>
                ...
                <p className="text-xs text-slate-400">Insights module ready. Generate to view the latest summary.</p>
              )}
            </div>
          </ModuleBoundary>
        ) : null}
        {activeTab === 'Image Analysis' ? (
          <ModuleBoundary moduleId="screensnap-insights">
            <div className="flex h-full flex-col gap-3 overflow-y-auto pr-1">
              {imageAnalyses.length === 0 ? (
                <p className="text-xs text-slate-400">Capture screenshots to build an annotated library of technique cues.</p>
              ) : null}
              {imageAnalyses.map((entry) => (
                <div key={entry.id} className="space-y-2 rounded-xl border border-purple-500/40 bg-purple-500/10 p-4 text-xs text-purple-100">
                  <div className="flex items-center justify-between text-[11px] uppercase tracking-wide text-purple-200">
                    <span>{entry.focus}</span>
                    <span>{entry.timestamp.toFixed(1)}s · {Math.round(entry.confidence * 100)}%</span>
                  </div>
                  <p className="text-sm font-semibold text-white">{entry.summary}</p>
                  <div>
                    <p className="text-[11px] font-semibold uppercase text-purple-200">Observations</p>
                    <ul className="mt-1 list-disc space-y-1 pl-4 text-left text-purple-100">
                  ...
                  <div>
                    <p className="text-[11px] font-semibold uppercase text-purple-200">Corrections</p>
                    <ul className="mt-1 list-disc space-y-1 pl-4 text-left text-purple-100">
                      {entry.corrections.map((item) => (
                        <li key={item}>{item}</li>
                      ))}
                    </ul>
                  </div>
